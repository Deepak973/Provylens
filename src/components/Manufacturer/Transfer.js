...
import { requestHistoryOfManufacturer } from "../../helper/manufacturerDistributorHelper";
  ...
  const getTransferData = async () => {
    ...
    const filteredData = reqHistory.map((val, index) => {
      return {
        ...
        productname: hexToString(val[1]["mp_name"]),
        p_description: hexToString(val[1]["mp_description"]),
        p_expiry_date: new Date(val[1]["mp_expiryDate"]).toDateString(),
        p_date_created: new Date(val[1]["mp_date"]).toDateString(),
        distributor_name: hexToString(val[2]["userName"]),
        distributor_address: val[0]["d_address"],
      };
    });
